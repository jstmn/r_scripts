...
class RobotConstants(object):
    ...
    def __init__(self, param_list = None ):
        ...
        self.MAX_ALLOWABLE_LEG_CONFIG_OFFSET = math.pi

        # Starting Configuration
        self.BASE_STATE_X_DELTA                  = param_list[0] if param_list else .3
        self.BASE_STATE_Y_DELTA                  = param_list[1] if param_list else .3
        self.BASE_STATE_Z_DELTA                  = param_list[2] if param_list else -.5 # This value is the delta z from the torso to the ground (/end effectors center point)
        ...
        self.END_RANGE_MULTIPLIER                = .955 # scale end effector range. .96: maximum range without ik failure
        self.SUPPORT_TRIANGLE_SAFETY_MARGIN     = .08         # scale support triangles. 0: no triangle margin, .07: maximum before ik failures

        # Turn Constants
        self.TORSO_YAW_ROTATE_ANGLE_DEG              = 12

        # Step Constants
        self.STEP_Z_MAX_HIEGHT                   = param_list[3] if param_list else .08
        self.TORSO_SHIFT_DELTA                   = param_list[4] if param_list else .15                               # TODO: This should be optimized or calculated.
        ...
        self.DUKE_MOTION_API_ENABLED             = False
        ...
        if self.ODE_PHYSICS_ENABLED:
            ...
            self.TORSO_SHIFT_TIME                    = param_list[7] if param_list else 5
            self.STEP_TIME                           = param_list[6] if param_list else 5
            self.TORSO_YAW_ROTATE_TIME               = 3

            self.TURNING_MIDSTEP_SLEEP_T = .9
            self.RESET_MIDSTEP_SLEEP_T = .9
            self.FORWARD_GAIT_SLEEP_T = .9

        elif self.DUKE_MOTION_API_ENABLED:
            self.INITIALIZATION_STEP_TIME            = 1
            self.RESET_LEG_STEP_TIME                 = 3
            self.TURN_GAIT_STEP_TIME                 = 2
            self.RESET_TORSO_SHIFT_TIME              = 3
            self.TORSO_SHIFT_TIME                    = param_list[7] if param_list else 1
            self.STEP_TIME                           = param_list[6] if param_list else 1
            self.TORSO_YAW_ROTATE_TIME               = 1
        ...
        self.AUTOMATED_GAIT_PROBIBILITY_DIST              = [5,1,1]                                 # higher the number relative to others, higher the prob. of this with the associated action being taken
        self.AVERAGE_COMPLETE_CYCLES                      = 5.5
