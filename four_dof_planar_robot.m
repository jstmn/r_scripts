...
function uirunner
    ...
    q = [xb_ yb_ yawb_ t_r1_ t_r2_ t_l1_ t_l2_ ];
    ...
    global xb_sym yb_sym yawb_sym theta1_sym theta2_sym 
    syms xb_sym yb_sym yawb_sym theta1_sym theta2_sym
    ...
    q_left_symbolic = [xb_sym yb_sym yawb_sym theta1_sym theta2_sym];
    Pl_endeff_symbolic = [ xb_sym - (Lb/2)*cos(yawb_sym) + L1*cos(theta1_sym + yawb_sym) - L2*cos(theta1_sym + theta2_sym + yawb_sym),...
                          yb_sym - (Lb/2)*sin(yawb_sym) + L1*sin(theta1_sym + yawb_sym) - L2*sin(theta1_sym + theta2_sym + yawb_sym)];
    ...
    function draw_sliders
        body_x = uicontrol('Parent',f,'Style','slider','Position',[75,100,600,25],'value',xb_, 'min',min_x, 'max',max_x);                  % [position: [left bottom width height]
        annotation(f,'textbox','String','X_b','units','pix', 'Position', [25,100,45,25],'Interpreter','Tex');
        body_x_val = uicontrol('Parent',f,'Style','text','Position',[695,100,45,25],'String',xb_,'BackgroundColor',bgcolor);
        body_x.Callback = @(es,ed) update_ui(1, body_x, body_x_val); 

        body_y = uicontrol('Parent',f,'Style','slider','Position',[75,75,600,25],'value',yb_, 'min',min_y, 'max',max_y);                  % [position: [left bottom width height]
        annotation(f,'textbox','String','Y_b','units','pix', 'Position', [25,75,45,25],'Interpreter','Tex');
        body_y_val = uicontrol('Parent',f,'Style','text','Position',[695,75,45,25],'String',yb_,'BackgroundColor',bgcolor);
        body_y.Callback = @(es,ed) update_ui(2, body_y, body_y_val); 

        body_yaw = uicontrol('Parent',f,'Style','slider','Position',[75,50,600,25],'value',yawb_, 'min',0, 'max',2*pi);                  % [position: [left bottom width height]
        annotation(f,'textbox','String','\Psi_b','units','pix', 'Position', [25,50,45,25],'Interpreter','Tex');
        body_yaw_val = uicontrol('Parent',f,'Style','text','Position',[695,50,45,25],'String',yawb_,'BackgroundColor',bgcolor);
        body_yaw.Callback = @(es,ed) update_ui(3, body_yaw, body_yaw_val); 

        tr1 = uicontrol('Parent',f,'Style','slider','Position',[800,100,600,25],'value',t_r1_, 'min',0, 'max',2*pi);                  % [position: [left bottom width height]
        annotation(f,'textbox','String','\theta_{r,1}','units','pix', 'Position', [750,100,45,25],'Interpreter','Tex');
        tr1_val = uicontrol('Parent',f,'Style','text','Position',[1425,100,45,25],'String',t_r1_,'BackgroundColor',bgcolor);
        tr1.Callback = @(es,ed) update_ui(4, tr1, tr1_val); 

        tr2 = uicontrol('Parent',f,'Style','slider','Position',[800,75,600,25],'value',t_r2_, 'min',0, 'max',2*pi);                  % [position: [left bottom width height]
        annotation(f,'textbox','String','\theta_{r,2}','units','pix', 'Position', [750,75,45,25],'Interpreter','Tex');
        tr2_val = uicontrol('Parent',f,'Style','text','Position',[1425,75,45,25],'String',t_r2_,'BackgroundColor',bgcolor);
        tr2.Callback = @(es,ed) update_ui(5, tr2, tr2_val); 

        tl1 = uicontrol('Parent',f,'Style','slider','Position',[800,50,600,25],'value',t_l1_, 'min',0, 'max',2*pi);                  % [position: [left bottom width height]
        annotation(f,'textbox','String','\theta_{l,1}','units','pix', 'Position', [750,50,45,25],'Interpreter','Tex');
        tl1_val = uicontrol('Parent',f,'Style','text','Position',[1425,50,45,25],'String',t_l1_,'BackgroundColor',bgcolor);
        tl1.Callback = @(es,ed) update_ui(6, tl1, tl1_val); 

        tl2 = uicontrol('Parent',f,'Style','slider','Position',[800,25,600,25],'value',t_l2_, 'min',0, 'max',2*pi);                  % [position: [left bottom width height]
        annotation(f,'textbox','String','\theta_{l,2}','units','pix', 'Position', [750,25,45,25],'Interpreter','Tex');
        tl2_val = uicontrol('Parent',f,'Style','text','Position',[1425,25,45,25],'String',t_l2_,'BackgroundColor',bgcolor);
...
%     
               ...
               % [x, y, yaw]
%             Pl_endeff = [ xb_sym - (Lb/2)*cos(yawb_sym) + L1*cos(theta1_sym + yawb_sym) - L2*cos(theta1_sym + theta2_sym + yawb_sym),...
%                           yb_sym - (Lb/2)*sin(yawb_sym) + L1*sin(theta1_sym + yawb_sym) - L2*sin(theta1_sym + theta2_sym + yawb_sym),...
%                           atan2(-sin(theta1_sym + theta2_sym + yawb_sym), -cos(theta1_sym + theta2_sym + yawb_sym) )];
